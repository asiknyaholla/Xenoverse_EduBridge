...
# ── PAGE ROUTES (Kept original from BS.py) ─────────────────────
@app.route('/')
...
# ── API ROUTES (Enhanced with features from app.py) ─────────────
...
def recommend():
    ...
    # Support both old style (from BS.py) and new rich recommendation (from app.py)
    ...
    # Backward compatibility for old BS.py frontend keys
    subject_map = {
        'math': 'maths',
